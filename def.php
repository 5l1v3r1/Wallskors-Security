...
if (extension_loaded('curl') || extension_loaded('dom'))
...
else
  {
    if (!extension_loaded('curl'))
      {
        echo $bold . $red . "\n[!] Le module curl est manquant...\nEssayez la commande 'fix' ou installez php-curl" . $cln;
      }
    if (!extension_loaded('dom'))
      {
        echo $bold . $red . "\n[!] Le module DOM est manquant...\nEssayez la commande 'fix' ou installez php-xml\n" . $cln;
      }
  }
thephuckinstart:
echo "\n";
userinput("Wallskors»Site Web Cible-» ");
$ip = trim(fgets(STDIN, 1024));
if ($ip == "help")
  {
    echo "\n\n[+] Wallskors Security Aide [+] \n\n";
    echo $bold . $lblue . "Commandes\n";
    echo "========\n";
    echo $fgreen . "[1] help:$cln Regarder le menu d'aide.\n";
    echo $bold . $fgreen . "[2] fix:$cln Install tout les modules requis (Utile si vous lancez l'outil pour la première fois)\n";
    echo $bold . $fgreen . "[3] URL:$cln Entrez le nom de domaine que vous voulez scanner (Format:www.sample.com / sample.com)\n";
    goto thephuckinstart;
  }
elseif ($ip == "fix")
  {
    echo "\n\e[91m\e[1m[+] Wallskors Security FiX MENU [+]\n\n$cln";
    echo $bold . $blue . "[+] Vérification de l'installation du module cURL...\n";
    if (!extension_loaded('curl'))
      {
        echo $bold . $red . "[!] cURL Module n'est pas installé ! \n";
        echo $yellow . "[*] Installation de cURL. (L'opération nécéssite les permissions administrateur donc vous pouvez être amené à entrer un mot de passe administrateur) \n" . $cln;
        system("sudo apt-get -qq --assume-yes install php-curl");
        echo $bold . $fgreen . "[i] cURL Installé. \n";
      }
    else
      {
        echo $bold . $fgreen . "[i] cURL est déjà installé, passage au module suivant\n";
      }
    echo $bold . $blue . "[+] Vérification de l'installation de php-XML ...\n";
    if (!extension_loaded('dom'))
      {
        echo $bold . $red . "[!] Le module php-XML n'est pas installé ! \n";
        echo $yellow . "[*] Installation de php-XML. (L'opération nécéssite les permissions administrateur donc vous pouvez être amené à entrer un mot de passe administrateurd) \n" . $cln;
        system("sudo apt-get -qq --assume-yes install php-xml");
        echo $bold . $fgreen . "[i] DOM Installé. \n";
      }
    else
      {
        echo $bold . $fgreen . "[i] php-XML est déjà installé. Toutes les vérifications ont étés effectuées ! ;) \n";
      }
    echo $bold . $fgreen . "[i] Tout s'est terminé correctement ! Relancer le programme ! \n";
    exit;
  }
elseif (strpos($ip, '://') !== false)
  {
    echo $bold . $red . "\n[!] (HTTP/HTTPS) Choisir le protocole :\n" . $CURLOPT_RETURNTRANSFER;
    goto thephuckinstart;
  }
elseif (strpos($ip, '.') == false)
  {
...
elseif (strpos($ip, ' ') !== false)
  ...
  {
    echo "\n";
    userinput("Protocole (1)http (2)https [1/2]");
...
scanlist:
    ...
    echo $bold . $blue . "
      ...
      $lblue Scan du site : " . $fgreen . $ipsl . $ip . $blue . "
      \n\n";
    echo $yellow . " [0]  Tâche simple$white (Nom du site, adresse IP, CMS, détection du Cloudflare, détection du Robots.txt)$yellow \n
	[1]  Whois Lookup \n
	[2]  Geo-IP Lookup \n
	[3]  Grab Banners \n 
	[4]  DNS Lookup \n 
	...
	[9]  SQLi Scanner$white (Vulnérabilité SQL.)$yellow \n 
	[10] Bloggers View$white (Recherche d'informations Blogger)(eng)$yellow \n 
	[11] WordPress Scan$white (Ne travaille que sur CMS WordPress)(eng)$yellow \n 7
	[12] Admin Page Scanner \n 
	[13] MX Lookup \n$magenta 
	[A]  Tâche complète (nécéssite un peu de temps) \n 
