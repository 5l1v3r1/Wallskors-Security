...
if args.host and args.user and args.file_password:
	...
	if os.path.exists(file):
		print(G+'Ouverture du fichier de mots de passe...'+W)
		text = open(file, 'r')
	else:
...
def connect():
	for line in text:
		...
		try:
			...
			print(G+time.ctime()+E+'Login -> '+F+user+E+' Mot de passe-> '+F+password+E)
			print(E+'Se connecter à ' +host+' ?')
			yes = input('O/n')
			if yes == 'o' or yes == 'O':
				print(G+'Connection ftp :'+E)
				time.sleep(1)
				os.system('ftp '+host)
				break
			else:
			...
			print(O+time.ctime()+E+' '+'Login incorrect-> '+B+user+E+' Mot de passe->'+B+password+E)
	sys.exit(1)
connect()